...
## Provide namespaces aligned with the folders 
func get_stitched_texture(namespaces: Array[String], texture_folders: Array[String]):
	var index = 0
	for folder in texture_folders: 
		var tex_dir = DirAccess.open(folder)
		...
		if tex_dir:
			...
			print("Unable to load the specified texture folder: ", folder)
		index+=1
